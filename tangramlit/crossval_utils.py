...
def get_cv_data(adata_sc, adata_st, k=10 ):
    """
...
def cross_validate_mapping(
    ...
    for train_genes, test_genes in tqdm(get_cv_data(adata_sc, adata_st, k), total=length):

        # Train mapper
        adata_map = lmu.map_cells_to_space_(
            adata_sc=adata_sc,
            adata_sp=adata_st,
            mode=mode,
